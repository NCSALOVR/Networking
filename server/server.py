...
def getList(input):
	received = binascii.unhexlify(input)
	return map(ord,received)

def cracker(captured,input): 	
	received = getList(captured)
	...
	out_0 = list_xor(cR,lR)
	out_1 = list_xor(cL,lL)
	out_2 = list_xor(cR,rR)
	out_3 = list_xor(cL,rL)
	print "".join(map(chr,out_0+out_1+out_2+out_3))
...
s.connect(("54.85.93.193",8888))
input = s.recv(2048)
print input
proof = input.split("with ")[1][0:16]
test = ""
done = 0
		...
		for c in range(0,256):
			for d in range(0,256):
				for e in range(0,256):
					test = proof+chr(a)+chr(b)+chr(c)+chr(d)+chr(e)
					ha = sha1()
					ha.update(test)
					if(test[0:16] == proof and ord(ha.digest()[-1]) == 0xff and ord(ha.digest()[-2]) == 0xff):
						done = 1
					if done == 1:
						break
				if done == 1:
...
s.send(test)
theString1 = s.recv(2048)
theString2 = s.recv(2048)
print theString1
print theString2			
toDecrypt = theString2[0:-1].split("decrypt: ")[1]
print toDecrypt
toSend = ""
for i in range(0,8):
	toSend+=chr(0)
...
print len(toSend)
s.send(toSend)
captured = s.recv(2048)
print captured[0:-1]
s.send(toSend)
captured = s.recv(2048)
print captured[0:-1]
cracker(captured[0:-1],toDecrypt)
f = [69,69,69,69]
f0 = fBox(f)
def fRev(f0):
	f_0 = (revrot3(f0[3])-f0[2]+256)%256
	f_3 = (revrot3(f0[0])-f0[1]-1+256)%256
	t0 = (revrot3(f0[1])-f0[2]+256)%256
	f_2 = t0^f_3
	t1 = (revrot3(f0[2])-1-t0+256)%256
	f_1 = t1^f_0
